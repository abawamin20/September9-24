...
const PagesList = (props: IPagesListProps) => {
  ...
  const [pageSize, setPageSize] = React.useState<number>(20); // Initially set to 20
    ...
    const checkIframeUrl = () => {
      ...
      if (iframe && iframe.contentWindow) {
        ...
        if (
          ...
          setSelectionDetails([]);
          getPages(catagory, columnInfos, pageIndex, pageSize);
        }
      }
    };
  ...
  const resetFilters = () => {
    ...
    fetchPages(pageSize, "Created", true, "", catagory, []);
  };
    ...
    return (
      ...
      pagesService
        ...
        .then((res: any) => {
          ...
          const finalPages = [...currentPages, ...res];
  ...
  const handlePageSizeChange = (e: any) => {
    ...
    getPages(catagory, columnInfos, 0, e.target.value, []);
  };
    ...
    const handleEvent = (e: any) => {
      if (columnInfos.length > 0) {
        ...
        if (selectedCategory && selectedCategory != "") {
          ...
          getPages(selectedCategory, columnInfos, pageIndex, pageSize);
    ...
    if (viewId !== selectedViewId) {
      ...
      getColumns(selectedViewId).then((col) => {
        if (catagory && catagory != "") {
          getPages(catagory, col, pageIndex, pageSize);
        }
      });
    }
  }, [selectedViewId]);
      ...
      {isLoading ? (
        ...
        <div onScroll={handleScroll}>
          <ReusableDetailList
            ...
            loadMoreItems={() => {
              hasNextPage &&
                getPages(catagory, columnInfos, pageIndex + 1, pageSize);
            }}
          />
        </div>
      )}
